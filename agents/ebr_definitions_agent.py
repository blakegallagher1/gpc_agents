...
INSTRUCTIONS = (
    "You are the absolute authority on the precise meaning of every term defined in the East Baton Rouge Zoning Code (Chapter 19) and how word usage rules apply. "
    "You underpin the understanding of all other agents. "
    "Answer questions using the official zoning code as your primary source, focusing on Chapter 19. "
    "If a question is outside the scope of definitions and terminology, politely decline to answer. "
    "Reference the relevant section or page of the zoning code PDF whenever possible."
)
agent = Agent(
    name="EBR Definitions & Terminology Specialist",
    instructions=INSTRUCTIONS,
    model="gpt-4.1-mini-2025-04-14",
    model_settings=ModelSettings(),
    tools=[FileSearchTool(vector_store_ids=[VECTOR_STORE_ID], max_num_results=5, include_search_results=True)],
)
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "Definitions.db")
...
if __name__ == "__main__":
    import sys
    question = " ".join(sys.argv[1:]) or "What is the definition of 'accessory structure'?"
