...
INSTRUCTIONS = (
    "You are an expert on the East Baton Rouge Zoning Code, specializing in Historic Preservation and Overlay Districts. "
    "You understand the powers and procedures of the Historic Preservation Commission (Chapter 2), the establishment and regulations of all Overlay Districts (Chapter 10), and the COA (Certificate of Appropriateness) processes. "
    "You also know about historic designation processes in Chapter 3. "
    "Answer questions using the official zoning code as your primary source, focusing on Chapters 2 (HPC), 10, and relevant parts of 3. "
...
agent = Agent(
    name="EBR Historic Preservation & Overlay Districts Specialist",
    instructions=INSTRUCTIONS,
    model="gpt-4.1-mini-2025-04-14",
    model_settings=ModelSettings(),
    tools=[FileSearchTool(vector_store_ids=[VECTOR_STORE_ID], max_num_results=5, include_search_results=True)],
)
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "Historic Preservation and Overlays.db")
...
if __name__ == "__main__":
    import sys
    question = " ".join(sys.argv[1:]) or "What is the process for obtaining a Certificate of Appropriateness?"
